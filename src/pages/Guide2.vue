    ...
    <div class="loading" v-if="isload">
      ...
      <x-dialog v-model="feedback" :dialog-style="{'overflow':'visible','border-radius':'8px', 'width': '70%'}">
        ...
        <div class="phone" style="padding:5px">
          ...
          <input v-model="phone" placeholder="请输入联系方式" type="tel" />
        </div>
        <div class="phone" style="padding:5px">
          <p style="text-align:left;font-size:.8rem;">联系人：</p>
          <input v-model="user" placeholder="联系人"/>
        </div>
        <div class="fd_text" style="padding:5px;font-size:.8rem;">
          <p style="text-align:left">意见或者问题：</p>
          <textarea placeholder="请写下您的宝贵建议或者问题，我们会认真改进的哦！" v-model="fdTT"></textarea>
...
<script type="text/javascript">
  ...
  import Vue from 'vue'
  var map, mapleLayer, geolocation, mylocation, line, movemk, warking, cluster
  let aCluster = 1
  var markers1 = []
  var markers2 = []
  var scnmarkers = []
  ...
  export default {
    ...
    methods: {
      ...
      add3d: function () {
        ...
        console.log(mapleLayer.getZooms())
        vm.isload = false
      },
      dituChange: function () {
        let vm = this
